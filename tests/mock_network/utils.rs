...
impl TestNetwork {
    pub fn count_sections(&self) -> usize {
        self.current_sections().count()
    }

    pub fn current_sections<'a>(&'a self) -> impl Iterator<Item = Prefix<XorName>> + 'a {
        self.nodes.iter().flat_map(|n| n.inner.prefixes()).unique()
    }
    ...
    pub fn poll_all(&mut self) -> bool {
        ...
        let network = self.nodes[0].network().clone();
    ...
    pub fn poll_and_resend(&mut self) {
        self.poll_and_resend_with_options(PollOptions::default())
    }

    /// Polls and processes all events, until there are no unacknowledged messages left.
    pub fn poll_and_resend_with_options(&mut self, mut options: PollOptions) {
        for _ in 0..MAX_POLL_CALLS {
            if (options.stop_predicate)(&self.nodes) {
                return;
            }

            let node_busy =
                |node: &TestNode| node.inner.is_elder() && node.inner.has_unpolled_observations();
            if self.poll_all() || self.nodes.iter().any(node_busy) {
            ...
            if let Some(continue_predicate) = options.continue_predicate.as_ref() {
                if continue_predicate(&self.nodes) {
                    continue;
                }
            }
    ...
    pub fn remove_nodes_which_failed_to_connect(&mut self, count: usize) -> usize {
        let failed_to_join: Vec<_> = self
            .nodes
            .iter_mut()
            .enumerate()
            .rev()
        ...
        let removed_nodes: Vec<_> = failed_to_join
            .iter()
            .map(|index| self.nodes.remove(*index).name())
            .collect();
        info!("Failed to be Added as Nodes: {:?}", removed_nodes);
        self.poll_and_resend();
        failed_to_join.len()
    }
    ...
    pub fn insert_node(&mut self, idx: usize, node: TestNode) {
        self.nodes.insert(idx, node);
    }

    pub fn remove_node(&mut self, idx: usize) -> Option<TestNode> {
        self.nodes.remove(idx)
    }

    pub fn shuffle<R: Rng>(&mut self, rng: &mut R) {
        rng.shuffle(&mut self.nodes)
    }

    pub fn create_connected_nodes(network: &Network, size: usize) -> Self {
        let mut result = Self::empty();

        // Create the seed node.
        result.add_node(TestNode::builder(network).first().create());
        let _ = result.poll_all();
        let endpoint = result.nodes[0].endpoint();
        info!("Seed node: {}", result.nodes[0].inner);

        // Create other nodes using the seed node endpoint as bootstrap contact.
        for _ in 1..size {
            let config = NetworkConfig::node().with_hard_coded_contact(endpoint);
            result.add_node(TestNode::builder(network).network_config(config).create());

            result.poll_and_resend();
            result.verify_invariant_for_all_nodes(&network);
        }
    ...
    pub fn create_connected_nodes_until_split(
        ...
        result.add_connected_nodes_until_split(network, prefix_lengths);
        result
    }
    ...
    pub fn add_connected_nodes_until_split(
        ...
        self.clear_all_event_queues(|_| {});

        // Start enough new nodes under each target prefix to trigger a split eventually.
        let min_split_size = unwrap!(self.nodes[0].inner.min_split_size());
        let prefixes_new_count = prefixes
            .iter()
            .map(|prefix| (*prefix, min_split_size))
            .collect_vec();
        self.add_nodes_to_prefixes(network, &prefixes_new_count);
        ...
        loop {
            ...
            if let Some(prefix_to_split) = found_prefix {
                self.add_node_to_section(network, &prefix_to_split);
            } else {
                break;
            }
        }
        ...
        self.clear_all_event_queues(|event| match event {
            Event::NodeAdded(..)
            | Event::NodeLost(..)
            | Event::TimerTicked
            | Event::ClientEvent(..)
        ...
        self.clear_relocation_overrides();
    ...
    pub fn add_connected_nodes_until_one_away_from_split(
        ...
        let (prefixes_and_counts, prefixes_to_add_to_split) =
            self.prefixes_and_count_to_split_with_only_one_extra_node(prefixes_to_nearly_split);

        self.add_connected_nodes_until_sized(network, &prefixes_and_counts);
    ...
    fn add_nodes_to_prefixes(&mut self, network: &Network, prefixes_new_count: &[PrefixAndSize]) {
        for (prefix, target_count) in prefixes_new_count {
            let num_in_section = self
                .nodes
                .iter()
            ...
            for _ in 0..to_add_count {
                self.add_node_to_section(network, prefix);
            }
        }
    }

    // Clear all event queues applying check_event to them.
    fn clear_all_event_queues(&mut self, check_event: impl Fn(Event)) {
        for node in self.nodes.iter_mut() {
            while let Ok(event) = node.try_next_ev() {
                check_event(event)
            }
    ...
    pub fn clear_relocation_overrides(&mut self) {
        for node in self.nodes.iter_mut() {
    ...
    fn prefixes_and_count_to_split_with_only_one_extra_node(
        ...
        let prefixes_to_add_to_split = prefixes
            .iter()
            .map(|prefix| self.prefix_half_with_fewer_nodes(prefix))
            .collect_vec();
    ...
    fn prefix_half_with_fewer_nodes(&self, prefix: &Prefix<XorName>) -> Prefix<XorName> {
    ...
    pub fn sort_nodes_by_distance_to(&mut self, name: &XorName) {
        let _ = self.poll_all(); // Poll
        self.nodes
            .sort_by(|node0, node1| name.cmp_distance(&node0.name(), &node1.name()));
    }
    ...
    pub fn verify_section_invariants_for_nodes(&self, elder_size: usize) {
        for node in self.nodes.iter() {
            Self::verify_section_invariants_for_node(node, elder_size);
        }
    }

    pub fn verify_section_invariants_between_nodes(&self) {
        #[derive(Debug)]
        ...
        for node in self.nodes.iter().filter(|node| node.inner.is_elder()) {
    ...
    pub fn verify_invariant_for_all_nodes(&mut self, network: &Network) {
        let elder_size = network.elder_size();
        self.verify_section_invariants_for_nodes(elder_size);
        self.verify_section_invariants_between_nodes();
    ...
    fn add_node_to_section(&mut self, network: &Network, prefix: &Prefix<XorName>) {
        let config =
            NetworkConfig::node().with_hard_coded_contacts(iter::once(nodes[0].endpoint()));
        ...
        let elder_size = network.elder_size();
        self.poll_and_resend_with_options(
            PollOptions::default()
        ...
        expect_any_event!(unwrap!(self.nodes.last_mut()), Event::Connected);
        assert!(
            prefix.matches(&self.nodes[self.nodes.len() - 1].name()),
    ...
    pub fn count_sections_members_if_split(&self) -> BTreeMap<Prefix<XorName>, usize> {
        let mut counts = BTreeMap::new();
        for pfx in self.nodes.iter().map(Self::node_prefix_if_split) {
    ...
    pub fn last_mut(&mut self) -> Option<&mut TestNode> {
        self.nodes.last_mut()
    }
}

impl Index<usize> for TestNetwork {
    type Output = TestNode;

    fn index(&self, i: usize) -> &TestNode {
        &self.nodes[i]
    }
}

impl IndexMut<usize> for TestNetwork {
    fn index_mut(&mut self, i: usize) -> &mut TestNode {
        &mut self.nodes[i]
    }
}
